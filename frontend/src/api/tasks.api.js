...
export async function createTaskComment(taskId, payload) {
  const { data } = await client.post(`/tenders/tasks/${taskId}/comments`, payload)
  return data.data
}

export async function getApprovalState(taskId) {
  const { data } = await client.get(`/tenders/tasks/${taskId}/approvals`)
  return data.data
}

export async function approveStep(taskId, stepId) {
  const { data } = await client.post(`/tenders/tasks/${taskId}/approvals/${stepId}/approve`)
  return data.data
}

export async function rejectStep(taskId, stepId, payload) {
  const { data } = await client.post(`/tenders/tasks/${taskId}/approvals/${stepId}/reject`, payload)
  return data.data
}
