...
async function createTender(req, res, next) {
  try {
    ...
    const tender = await service.createTender(req.body, req.user.userId, req.tenantId)
...
async function updateTender(req, res, next) {
  try {
    const tender = await service.updateTender(Number(req.params.id), req.body, req.tenantId)
    res.json({ success: true, data: tender })
  } catch (err) {
    if (err.statusCode) {
      return res.status(err.statusCode).json({
...
async function deleteTender(req, res, next) {
  try {
    await service.deleteTender(Number(req.params.id), req.tenantId)
...
async function changeStatus(req, res, next) {
  try {
    ...
    await service.changeStatus(Number(req.params.id), status, req.tenantId)
...
async function assignTender(req, res, next) {
  try {
    ...
    await service.assignTender(Number(req.params.id), assignments, req.tenantId)
